...
你写了"先安抚再讲道理会被对方读成敷衍不走心，所以两类内容必须在使用时机上强制分开"，系统是这样实现的：

**Prompt 层（硬约束）：**
- 要求严格按 `## 当场回复候选` / `## 复盘参考` 两个固定标题分隔
- Block A：只情绪确认+延后表态，2-3 个版本，每版≤2句，**禁止任何建议、说理、引用历史、方案**——出现=不合格输出
- Block B：针对该人物的历史互动模式样本（`interaction_patterns`，observation_count<3 时明确写"样本不足，谨慎参考"）、过去试过的应对+结果、可能的谈判方向（带 event_id 引用 + "基于单方面数据推断，非对方真实立场"声明）。绝不直接转发给对方

**前端层（物理隔离）：**
- Block A 默认展开，绿色主题，大号显示 — 当场决策用
- Block B 折叠在 `<details>` 里，紫色，需要手动点开 — 不干扰当场情绪
- Turn 下方挂"记录这次结果"小表单：事后把"发出去的回复+对方实际反应"POST 到 `/api/interaction_patterns/upsert`

**Upsert 逻辑（回填积累样本）：**
按 `person + trigger_context` 匹配已有记录：
- 匹配到：`observation_count+1`，`response_tried` 和 `outcome` 用 ` | ` 分隔追加（保留历次记录，不覆盖）
- 没匹配：新建一条，count=1

这就是"冷启动难"的解决办法：前期 Block B 全是"暂无样本"，但每次事后回填一次，下一轮同情境就能看到自己试过什么、结果如何——是自己对自己的行为学习记录。
...
5. **Block A 可能越界**：即使 prompt 写了"禁止说理"，AI 可能偷偷在"我理解你"之后塞一句"但其实可以……"。这属于 LLM 本身对齐局限，产品只能用固定格式+人工审查双重保险，没法单靠 prompt 100% 杜绝。
